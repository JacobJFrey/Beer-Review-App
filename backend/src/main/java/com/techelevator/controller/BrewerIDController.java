...
@RestController
@CrossOrigin
public class BrewerIDController {

	private BreweryDAO breweryDAO;
	
	public BrewerIDController(BreweryDAO breweryDAO ) {
...
//	}
	...
	//Get List of Breweries for Brewer
	@PreAuthorize("hasRole('ADMIN') || hasRole('BREWER')")
	@ResponseStatus(HttpStatus.OK) 
	@RequestMapping(value = "/brewer/{id}", method = RequestMethod.GET)
	public List<Brewery> breweryList(@PathVariable Long id) {
		List<Brewery> breweryList = breweryDAO.findMyBreweries(id);
		return breweryList;
    }
