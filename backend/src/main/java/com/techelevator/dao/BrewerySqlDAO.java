...
public class BrewerySqlDAO implements BreweryDAO {
    ...
    public List<Brewery> findMyBreweries(Long userId) {
        List<Brewery> breweries = new ArrayList<>();
        String sql = "SELECT breweries.brewery_id FROM breweries JOIN brewers ON brewers.brewery_id = breweries.brewery_id WHERE brewers.user_id = ?;";
    ...
    public Brewery create(Brewery newBrewery) {
        String sqlCreateBrewery = "INSERT INTO breweries (brewery_id, brewery_name, contact_info, phone, website, open_time, close_time, address, history) " +
        						  "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        newBrewery.setId(getNextBreweryId());
        jdbcTemplate.update(sqlCreateBrewery, newBrewery.getId(), newBrewery.getBreweryName(), newBrewery.getContactInfo(), newBrewery.getPhone(), newBrewery.getWebsite(), newBrewery.getOpenTime(), newBrewery.getCloseTime(), newBrewery.getAddress(), newBrewery.getHistory());
